...
logger = logging.getLogger("CIRAF.model")
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self.attn = nn.MultiheadAttention(d_model, n_head) # Attention(d_model, n_head)  #
    ...
    def forward(self, x: torch.Tensor):
        x_, attn_score = self.attention(self.ln_1(x))
        x = x + x_
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.positional_embedding = nn.Parameter(torch.randn(self.context_length, transformer_width))
    ...
    def encode_image(self, image):
        x, attn_scores = self.visual(image.type(self.dtype))

        # x2 = x.permute(0, 2, 1)
        # x2 = self.dict_conv(x2)
        # w = F.softmax(x @ x2, dim=1)
        # x2 = w @ x2.permute(0, 2, 1)
        # x = x + x2

        # x = self.fusion_bn(x.permute(0, 2, 1)).permute(0, 2, 1)
...
def convert_weights(model: nn.Module):
    ...
    def _convert_weights_to_fp16(l):
        if isinstance(l, (nn.Conv1d, nn.Conv2d, nn.Linear, nn.BatchNorm1d)):
            l.weight.data = l.weight.data.half()
            if l.bias is not None:
                l.bias.data = l.bias.data.half()

        if isinstance(l, nn.BatchNorm1d):
            l.running_mean = l.running_mean.half()
            l.running_var = l.running_var.half()
